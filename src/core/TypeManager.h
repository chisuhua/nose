...
public:
    ...
    template <typename T>
    static PropertyMeta createMetadata(std::string type_name)
    {
        PropertyMeta md;
        md.name_ = type_name;
        ...
        for_each(refl::reflect<T>().members, [&](auto member) {
            if constexpr (refl::descriptor::has_attribute<Property>(member)) {
                ...
                if (auto propIter = properties.find(member.name.str()); propIter != properties.end()) {
                    member(*instance) = std::visit([&](auto&& arg) -> member_type {
...
public:
    ...
    void registerType() {
        ...
        auto type_str = std::string(type_name.c_str());
        metadata[type_str] = PropertyMeta::createMetadata<T>(type_str);
    }
